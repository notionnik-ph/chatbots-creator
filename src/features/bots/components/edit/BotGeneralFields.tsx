...
export type BotFieldUpdater = <K extends keyof BotFormData>(field: K, value: BotFormData[K]) => void;
  ...
  return (
    <section className="space-y-6">
      <header>
        ...
        <h2 className="text-xl font-semibold text-text-primary">Give your chatbot an identity</h2>
        <p className="mt-1 text-sm text-text-muted">Set the name, website, welcome message, and placement for this bot.</p>
      </header>
      ...
      <label className="block">
        ...
        <select
          className="input"
          value={form.position}
          onChange={(event) => update("position", event.target.value as BotFormData["position"])}
        >
      ...
      <div className="grid gap-3 border-t border-border pt-5 sm:grid-cols-3">
        {[
          ...
          [MessageSquareText, "Welcome message", "Start every chat with a clear greeting."],
          [Webhook, "Webhook optional", "Use an external URL only when your flow needs one."],
          ...
          return (
            <div key={String(title)} className="rounded-xl bg-surface p-3">
              ...
              <p className="mt-1 text-xs leading-relaxed text-text-muted">{description as string}</p>
            </div>
          );
        })}
      </div>
