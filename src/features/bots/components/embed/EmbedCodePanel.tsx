    ...
    <section className="space-y-5">
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-start">
        <div>
          ...
          <h2 className="text-xl font-semibold text-text-primary">Embed this chatbot on your website</h2>
          <p className="mt-1 text-sm text-text-muted">
            Copy the snippet and paste it before the closing <code className="rounded bg-surface px-1.5 py-0.5 text-primary">&lt;/body&gt;</code> tag.
          </p>
        </div>
        <button type="button" className="btn-secondary shrink-0 px-4 py-2" onClick={() => void copyCode()}>
      ...
      <div className="relative overflow-hidden rounded-xl border border-border bg-[#020205]">
        <div className="flex items-center justify-between border-b border-border bg-surface px-4 py-2">
          ...
          <span className="text-[10px] uppercase tracking-[0.16em] text-text-muted">Ready to paste</span>
        </div>
        <pre className="overflow-x-auto p-5 text-xs leading-relaxed text-text-secondary"><code>{snippet}</code></pre>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-border bg-surface p-4">
          ...
          <ol className="space-y-1.5 text-sm text-text-secondary">
            ...
            <li>2. Open your website custom-code area.</li>
            <li>3. Paste it before the closing body tag.</li>
            <li>4. Save and publish your website.</li>
          </ol>
        </div>
        <div className="rounded-xl border border-border bg-surface p-4">
          <p className="mb-2 text-sm font-semibold text-text-primary">Standalone widget preview</p>
          <a
          ...
          <p className="mt-3 text-xs text-text-muted">Bot ID: <code className="text-primary">{bot.refId}</code></p>
        </div>
      </div>
    </section>
  );
