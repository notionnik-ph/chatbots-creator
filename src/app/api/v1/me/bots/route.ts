...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log("[API] GET /api/v1/me/bots success, returning", {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("[API] POST /api/v1/me/bots bot created:", {
      botRef: result.bot.refId,
    });
      ...
      return Response.json(
        {
          ...
          code: billingError.code ?? "BILLING_LIMIT_REACHED",
        },
        {
          status: billingError.status ?? 403,
        },
      );
    }
    return toErrorResponse(error);
