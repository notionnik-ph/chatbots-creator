...
export default function ConversationsPage() {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/v1/me/bots/${botRef}?include=conversations`, {
      headers: { Authorization: `Bearer ${session.access_token}` },
    })
      .then(async (response) => {
        const body = await response.json();
        if (!response.ok) throw new Error(body.error);
        return body.data as { bot: { name: string }; conversations: ConversationLog[] };
      })
      .then((data) => {
  ...
  return (
    <div className="mx-auto max-w-5xl p-6 sm:p-8">
      ...
      <header className="mt-7 mb-8">
        ...
        <p className="mt-1 text-text-secondary">Recent chats handled by {name}.</p>
      </header>

      {error && (
        <div className="mb-5 rounded-xl border border-red-500/20 bg-red-500/10 px-4 py-3 text-sm text-red-300">{error}</div>
      )}
      ...
      <div className="space-y-5">
        {items.length ? (
            ...
            <article key={item.id} className="card overflow-hidden p-0">
              ...
              <div className="space-y-4 p-5">
                <div className="flex items-start gap-3">
                  <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-surface text-text-secondary">
                    <UserRound size={15} />
                  </span>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="mb-1 text-xs font-semibold uppercase tracking-wide text-text-muted">{name}</p>
          ...
          <EmptyState
            ...
            description="Conversation logs will appear here after visitors use this bot on your website."
